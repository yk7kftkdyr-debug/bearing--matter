function ff3(datafromvb,loadi,ttt1 )   % 求解 承载区  滚动体的1  3 两个方程组成的 方程组
...
loadj=length(loadi);
for i=1:loadj
Wo(i)=(W2*(1-gama)-W1*(1+gama))/2;     Wx(i)=(W2-Wo(i))*(1-gama)/gama;
end

Z2=pp(3*loadj+1);Y2=pp(3*loadj+2);sitay=pp(3*loadj+3);sitaz=pp(3*loadj+4);
for i=1:loadj
    z2(i)=0;z4(i)=0;    
    delta1(i)=pp(i);  beita(i)=pp(loadj+i);  afa(i)=pp(2*loadj+i);
...
%*****************************************************************************************   
    ...
    for k=1:150
       ...
       delta1kafa=sqrt((Dr1/2)^2-((-lenroller/2+(k-0.5)*lenroller/150)*sin(sitaz*cos(sita(i))-afa(i)) )^2   )-Dr1/2;
       delta2kafa=sqrt((Dr2/2)^2-((-lenroller/2+(k-0.5)*lenroller/150)*sin(sitaz*cos(sita(i))-afa(i)) )^2   )-Dr2/2;
...
%*****************************************************************************************          
       ...
       %  动压力计算！！
         niandu01(i)=0.132*sinh(925.19/(-148.85+sita0+273))*oilden/1e6;          %sita0 度时的粘度，wys第40页
         niandu02(i)=0.132*sinh(925.19/(-148.85+sita0+273))*oilden/1e6; % sita0度时的粘度，wys第40页
         U1(i)=0.5*0.5*Dm*((1+gama)*(W1+Wo(i))+gama*Wx(i));
         U2(i)=0.5*0.5*Dm*((1-gama)*(W2-Wo(i))+gama*Wx(i));
         FP1e(i)=FP1e(i)+ lenroller/150*E1*Rr1*1.43*(1+gama)*(niandu01(i)*U1(i)/(2*E1*Rr1))^0.71 *(lenroller/2-(k-0.5)*thickness);
         FP2e(i)=FP2e(i)+ lenroller/150*E2*Rr2*1.43*(1+gama)*(niandu02(i)*U2(i)/(2*E2*Rr2))^0.71 *(lenroller/2-(k-0.5)*thickness);
...
zz1=[ z2';z4';z5';z7;z6;z8;z9 ];save zz1;

result112=0;
for i=1:3*loadj+4
    result112=result112+zz1(i)^2;
end
save result112

save Q1;save Q2;
% STAGE2C1A_FF3_SNAPSHOT_BEGIN
if isfield(micro_config,'roughness') && isstruct(micro_config.roughness) && isfield(micro_config.roughness,'enabled') && isfield(micro_config.roughness,'feedback_level') && isscalar(micro_config.roughness.enabled) && (islogical(micro_config.roughness.enabled) || isnumeric(micro_config.roughness.enabled)) && logical(micro_config.roughness.enabled) && isscalar(micro_config.roughness.feedback_level) && isnumeric(micro_config.roughness.feedback_level) && micro_config.roughness.feedback_level == 1
    save('roller_feedback_terms.mat','loadi','Q1','Q2','oilh1_legacy','oilh2_legacy','oilh1_used','oilh2_used','delta1k','delta2k','widthcontact11','widthcontact22','lenroller');
end
% STAGE2C1A_FF3_SNAPSHOT_END
        %  接触载荷  ！！！
save QQ1;save QQ2;      %  载荷强度 分布情况！！！
