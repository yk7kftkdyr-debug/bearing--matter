...
if isfield(micro_config,'roughness') && micro_config.roughness.enabled && isfield(micro_config.roughness,'feedback_level') && micro_config.roughness.feedback_level==1
    ballId=loadi(i); correction=micro_config.roughness.feedback.ball;
    assert(numel(correction.delta_h_outer)>=ballId && numel(correction.delta_h_inner)>=ballId,'JffLOAD:RoughnessFeedback','Missing fixed ball film correction.');
    oilh1(i)=oilh1(i)+correction.delta_h_outer(ballId); oilh2(i)=oilh2(i)+correction.delta_h_inner(ballId);
    assert(oilh1(i)>0 && oilh2(i)>0,'JffLOAD:RoughnessFeedback','OUT_OF_MODEL_DOMAIN: nonpositive corrected film.');
end
...
if isfield(micro_config,'roughness') && micro_config.roughness.enabled && ...
    ...
    assert(feedback_state.feedback_active,'JffLOAD:RoughnessFeedback','Normal feedback state is inactive.');
