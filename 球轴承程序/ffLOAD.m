...
inner_gap_shift = zeros(1, loadj);
if micro_config.debris.enabled
    frozen_ud = reshape(micro_config.debris.ud_frozen_m, 1, []);
    frozen_loadi = reshape(micro_config.debris.ud_frozen_loadi, 1, []);
    if numel(frozen_ud) == numel(frozen_loadi)
        [matched, frozenIndex] = ismember(loadi, frozen_loadi);
        inner_gap_shift(matched) = micro_config.debris.eta_d * frozen_ud(frozenIndex(matched));
    elseif numel(frozen_ud) == loadj
        inner_gap_shift = micro_config.debris.eta_d * frozen_ud;
    end
end
%初始的假设值！！！
...
for i=1:loadj       %共loadj个球循环
    sita(i)=2*pi*(loadi(i))/n + micro_config.debris.cage_phase_rad;     %求各个球的方位角
    ...
    U2(i)=abs((W2-Wo)*Dm/4+0.5*(abs(Wx)*cos(a0)-(W2-Wo)*cos(a0))*(Dw/2));
...
% oilh2(16)=0.116*10^(-6);oilh2(17)=0.115*10^(-6);oilh2(18)=0.114*10^(-6);
    ...
    delta1(i)=sqrt((xx(i))^2+y(i)^2)-(f1-0.5)*Dw-oilh1(i);%崔立博士论文eq.2-15，接触变形中涉及未知变量x(i)，r(i)，内圈接触还涉及X2，Y2,Z2,sitaz,sitay
    delta2(i)=sqrt(( ((f1+f2-1)*Dw )*sin(a0)+X2+R222*(sitaz*sin(sita(i))+sitay*cos(sita(i)))-(xx(i)))^2+( ((f1+f2-1)*Dw )*cos(a0)+Z2*cos(sita(i))+Y2*sin(sita(i))-y(i))^2)-((f2-0.5)*Dw )-oilh2(i)+inner_gap_shift(i);
   ...
   % delta2(i)=sqrt(( ((f1+f2-1)*Dw )*sin(a0)+X2+R222*(sitaz*sin(sita(i))+sitay*cos(sita(i)))-(xx(i)))^2+( ((f1+f2-1)*Dw )*cos(a0)+Z2*cos(sita(i))+Y2*sin(sita(i))-y(i))^2)-((f2-0.5)*Dw )-0.502*10^(-6);
     delta1(i)=sqrt((xx(i))^2+y(i)^2)-(f1-0.5)*Dw-oilh1(i);
     delta2(i)=sqrt(( ((f1+f2-1)*Dw )*sin(a0)+X2+R222*(sitaz*sin(sita(i))+sitay*cos(sita(i)))-(xx(i)))^2+( ((f1+f2-1)*Dw )*cos(a0)+Z2*cos(sita(i))+Y2*sin(sita(i))-y(i))^2)-((f2-0.5)*Dw )-oilh2(i)+inner_gap_shift(i);
    ...
    while (delta2(i)<=0)
        if  xx(i)<1e-10   |   sqrt(xx(i)^2+y(i)^2)-(f1-0.5)*Dw-oilh1(i)<1e-10                    %如果xx(i)<0, 要停止改变x(i) r(i),而是改变X2
            while(delta2(i)<=0)
                X2=X2+1e-7;%调节X2
                delta2(i)=sqrt(( ((f1+f2-1)*Dw    )*sin(a0)+X2+R222*(sitaz*sin(sita(i))+sitay*cos(sita(i)))-(xx(i)))^2+( ((f1+f2-1)*Dw    )*cos(a0)+Z2*cos(sita(i))+Y2*sin(sita(i))-y(i))^2)-((f2-0.5)*Dw )-oilh2(i)+inner_gap_shift(i);
            end
        ...
        if   sqrt((xx(i))^2+y(i)^2)-(f1-0.5)*Dw-oilh1(i)<1e-10            %如果改变xx(i) y(i)使得delta1(i)<0 则要停止改变x(i) r(i),而是改变X2
            delta1(i)=sqrt((xx(i))^2+y(i)^2)-(f1-0.5)*Dw-oilh1(i);
            if delta1(i)<=0
                xielv1=y(i)/xx(i);
                xielv=((f1+f2-1)*Dw*cos(a0)+Z2*cos(sita(i))+Y2*sin(sita(i)))/( (f1+f2-1)*Dw*sin(a0)+X2+R222*(sitaz*sin(sita(i))+sitay*cos(sita(i))));
...
save q1q2a1a2
save contact_state delta1 delta2 inner_gap_shift
